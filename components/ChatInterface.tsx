...
import { useOutputLanguage, useAgentProgress, type AgentResult } from "@/components/ChatRuntimeProvider";
...
export function ChatInterface() {
  const { language, setLanguage, options } = useOutputLanguage();
        ...
        <div className="shrink-0 border-t border-slate-200 bg-white p-3 sm:p-4">
          <ComposerPrimitive.Root className="max-w-4xl mx-auto">
            <div className="flex flex-col gap-2 sm:gap-3">
              {/* Cara 2: Template Chat - Form untuk mengisi brand & campaign info */}
              <CampaignTemplateForm 
                onMessageSubmit={submitMessageProgrammatically}
              />
              
              {/* Cara 1: Chat Langsung - Input text langsung untuk mengirim pesan */}
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <ComposerPrimitive.Input
                  ref={composerInputRef}
                  placeholder="Ketik 'ping' untuk uji koneksi — atau kirim brief kampanye (crew butuh 1–2 menit)"
              ...
              <div className="flex items-center gap-2 text-slate-600 text-sm">
                <label htmlFor="output-language" className="shrink-0">
                  Output language:
                </label>
                <select
                  id="output-language"
                  ...
                  className="rounded-lg border border-slate-300 px-3 py-2 text-sm bg-white focus:border-bagana-primary focus:outline-none focus:ring-1 focus:ring-bagana-primary/20"
                >
                  {options.map((opt) => (
