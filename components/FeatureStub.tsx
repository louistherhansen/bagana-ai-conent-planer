...
export function FeatureStub({
  ...
  const phaseColors = {
    P0: "bg-emerald-100 text-emerald-800",
  ...
  const content = (
    <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm transition-shadow hover:shadow-md focus-within:ring-2 focus-within:ring-bagana-primary/20">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span
          className={`inline-block rounded-md px-2 py-0.5 text-xs font-medium ${phaseColors[phase]}`}
        >
          {phase}
        </span>
        {prdRef && (
          <span className="text-xs text-slate-400">({prdRef})</span>
        )}
      </div>
    </div>
  );
